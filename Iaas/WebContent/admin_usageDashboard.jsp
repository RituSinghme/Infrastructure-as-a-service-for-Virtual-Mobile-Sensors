...
$(function () {
	...
	<%
	usg = us.executeQuery("select sum(hour(timediff(IFNULL(str_to_date(end_time,'%m/%d/%Y %h:%i %p'), current_timestamp()), str_to_date(start_time,'%m/%d/%Y %h:%i %p'))) ) as sum from user_sensor group by date(str_to_date(start_time,'%m/%d/%Y %h:%i %p')); ");	
	if (usg.next() == true)
	{
	String us_id ;
	String usage = "";
   ...
   // System.out.println(date);
	...
	<%
	dte = dt.executeQuery("select date(str_to_date(start_time,'%m/%d/%Y %h:%i %p')) as date from user_sensor group by date(str_to_date(start_time,'%m/%d/%Y %h:%i %p')) order by date(str_to_date(start_time,'%m/%d/%Y %h:%i %p'));");
	...
	<%
	od = st.executeQuery("select count(sensor_id) as total from user_sensor group by date(str_to_date(start_time,'%m/%d/%Y %h:%i %p')) order by date(str_to_date(start_time,'%m/%d/%Y %h:%i %p'));");
	//out.println(od.next());
...
<% type = tp.executeQuery("select type from sensor s join user_sensor u  on s.location_id = u.location_id group by type ;");
//out.println(od.next());
if (type.next() == true)
{
String tp_id ;
...
<% rs = ab.executeQuery("select count(sensor_id) as total from sensor s join user_sensor u  on s.location_id = u.location_id group by type ;");
//out.println(od.next());
if (rs.next() == true)
{
String ph_id ;
...
<% ps = xz.executeQuery("select count(sensor_id) as total from sensor s join user_sensor u  on s.location_id = u.location_id group by city ;");
//out.println(od.next());
if (ps.next() == true)
{
String ph_id ;
...
<% city = ct.executeQuery("select city from sensor s join user_sensor u  on s.location_id = u.location_id group by city ;");
//out.println(od.next());
if (city.next() == true)
{
String ct_id ;
...
<% gh = gp.executeQuery("select sum(hour(timediff(IFNULL(str_to_date(end_time,'%m/%d/%Y %h:%i %p'), current_timestamp()), str_to_date(start_time,'%m/%d/%Y %h:%i %p'))) ) * 2000 as total_mb, date(str_to_date(start_time,'%m/%d/%Y %h:%i %p')) as date from user_sensor group by date(str_to_date(start_time,'%m/%d/%Y %h:%i %p'));");
//out.println(od.next());
if (gh.next() == true)
{
String ph_id ;
...
<% pc = pe.executeQuery("select count(sensor_id) as total, date(str_to_date(start_time,'%m/%d/%Y %h:%i %p')) as date from sensor s join user_sensor u  on s.location_id = u.location_id where  status='running' group by date(str_to_date(start_time,'%m/%d/%Y %h:%i %p'));");
//out.println(od.next());
if (pc.next() == true)
{
String ph_id ;
